// counter.c
// more advanced counter example from 'how to write pure data externals'
// nks sep 09
...
typedef struct _counter {
  ...
  t_int i_count;
  t_float step;
  t_int i_down, i_up; // bounds
  t_outlet *f_out, *b_out;
} t_counter;

void counter_bang(t_counter *x) {
  t_float f = x->i_count; // isn't count an int?
  t_int step = x->step; // isn't step a float?
...
void counter_setup(void) {
  ...
  class_addmethod(counter_class,
    (t_method)counter_set, gensym("set"), A_DEFFLOAT, 0);
  class_addmethod(counter_class,
